...
public interface PetGroupMedicalControllerSwagger {
    ...
    @ApiResponse(
            ...
            content = @Content(
                    schema = @Schema(implementation = PetMedicalInfoResponse.class))
    )
    @ErrorCode401
    @ErrorCode404(description = "AI 의사 권장사항 생성 중")
    @ErrorCode500
    ResponseEntity<PetMedicalInfoResponse> getGroupMedicalInfo(
    ...
    @ApiResponse(
            ...
            description = "조회 성공",
            content = @Content(
                    schema = @Schema(implementation = PetMedicalStatisticsResponse.class)
            )
    )
    ResponseEntity<PetMedicalStatisticsResponse> findGroupDieseaseStatistics(
            @Parameter(description = "인증된 사용자 정보 (Access Token 기반)", hidden = true)
            Member member
    );
